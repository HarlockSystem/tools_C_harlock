...
intarray intarray_clone_intheap_sort_by_min(intarray I)
{
  if(I->len == 0)
    {
      fprintf(stderr,"error intarray_clone_intheap_sort_by_min intarray is empty");
      return NULL;
    }
  int i;
  intheap intheap_sort = intheap_create_empty(I->len);
  
  intheap_sort_by_min(intheap_sort, I);
  
  
  intarray J = empty_intarray_create(I->len);
  for(i = 0; i < I->len; i++)
    {
      intarray_add(J, intheap_get_head_value(intheap_sort));
      intheap_del_head_val_by_min(intheap_sort);
    }
  
  intheap_destroy(&intheap_sort);
  return J;
}

void intarray_D_intheap_sort_by_min(intarray I)
{
  if(I->len == 0)
    {
      fprintf(stderr,"error intarray_D_intheap_sort_by_min intarray is empty");
      return;
    }
  int i;
  intheap intheap_sort = intheap_create_empty(I->len);
  ...
  for(i = 0; i < I->len; i++)
    {
      I->data[i] = intheap_get_head_value(intheap_sort);
      intheap_del_head_val_by_min(intheap_sort);
    }
  intheap_destroy(&intheap_sort);
}
...
intarray intarray_clone_intheap_sort_by_max(intarray I)
{
  if(I->len == 0)
    {
      fprintf(stderr,"error intarray_clone_intheap_sort_by_max intarray is empty");
      return NULL;
    }
  int i;
  intheap intheap_sort = intheap_create_empty(I->len);
  
  intheap_sort_by_max(intheap_sort, I);
  ...
  for(i = 0; i < I->len; i++)
    {
      intarray_add(J, intheap_get_head_value(intheap_sort));
      intheap_del_head_val_by_max(intheap_sort);
...
void intarray_D_intheap_sort_by_max(intarray I)
{
  if(I->len == 0)
    {
      fprintf(stderr,"error intarray_D_intheap_sort_by_max intarray is empty");
      return;
    }
  int i;
  intheap intheap_sort = intheap_create_empty(I->len);
...
intheap intheap_create_empty(int alloc)
{
  intarray I = empty_intarray_create(alloc);
  return I;
}
 
intheap intheap_ext_create(int len)
{
  intarray j = ext_intarray_create(len, len);
  return j;
}

intheap intheap_destroy(intheap* H)
{
  intarray_destroy(*H);
  (*H) = NULL;
}

bool intheap_is_empty( intheap H)
{
  return (H->len == 0);
}

int intheap_count_children(intheap H, int i)
{
  int lc = (i*2)+1;
  if (lc >= H->len)
    return 0;
...
void intheap_sort_by_min(intheap H, intarray I)
{
  int i;
  for (i = 0; i < I->len; i++)
    {
      ...
      while(intheap_has_parent(H, index))
	{
	  int parent_index = intheap_parent_index(index);
	  if (H->data[index] < H->data[intheap_parent_index(index)])
	    {
	      int_swaph(H->data + index, H->data + intheap_parent_index(index));
	      index = parent_index;
	     }
	  else
	    break;
...
void intheap_sort_by_max(intheap H, intarray I)
{
  int i;
...
int intheap_parent_index(int x)
{
// impair (n-1)/2 pair (n/2)-1
  return (x -1)/2;
}

int intheap_left_child_index(int x)
{
// (nx2)+1 impair
  
  return (2  * x + 1);
}
int intheap_right_child_index(int x)
{
// (n+1)x2 pair
  return (2 * x + 2) ;
}
intdata intheap_get_parent(intheap H, int x)
{
  return intarray_get(H, intheap_parent_index(x));
}

intdata intheap_get_head_value(intheap H)
{
      return intarray_get(H, 0);
}

void intheap_del_head_val_by_min(intheap H)
{
if (H->len ==  1)
    {
      H->len = 0;
 ...
 else
   {
     ...
     index = 0;
     H->len--;
     while(intheap_count_children( H, index) > 0)
       {
	 int smallest_child_index = intheap_get_small_child_index(H, index);
	 if (H->data[index] > H->data[intheap_get_small_child_index(H,index)])
	   {
...
void intheap_del_head_val_by_max(intheap H)
{
 ...
 if (H->len ==  1)
 ...
 else
   {
     ...
     while(intheap_count_children( H, index) > 0)
       {
	 int biggest_child_index = intheap_get_big_child_index(H, index);
...
int intheap_get_small_child_index(intheap H,int x)
{
  if (intheap_count_children(H, x) == 1)
    return intheap_left_child_index(x);
  else
    {
      if (intarray_get(H,intheap_left_child_index(x)) > (intarray_get(H,intheap_right_child_index(x))))
	return intheap_right_child_index(x);
...
int intheap_get_big_child_index(intheap H,int x)
{
  if (intheap_count_children(H, x) == 1)
    return intheap_left_child_index(x);
  else
...
bool intheap_is_parent_smaller(intheap H, int x)
{
  return ( intarray_get(H, x) > intarray_get(H , intheap_parent_index( x)));
}
 
bool intheap_is_parent_bigger(intheap H, int x)
{
  return ( intarray_get(H, x) < intarray_get(H , intheap_parent_index( x)));   
}

bool intheap_has_parent(intheap H, int x)
{
  return (x > 0);
} 
